...
public class DNSGAII extends DynamicAlgorithm {
	...
	public List<SolutionSet> execute() throws JMException, ClassNotFoundException {
		...
		*/
		int fc = 20;
		int nc = 30;
		List<SolutionSet> eachPopulations = new ArrayList<SolutionSet>();
		...
		while (evaluations < maxEvaluations) {
			//动态变化30次，每次静态迭代20次，把第20次的PF存在dynamicPopulationSets中，则dynamicPopulationSets的size应该为30
			if((generations) % fc == 0){
				ranking = new Ranking(population);
				dynamicPopulationSets.add(ranking.getSubfront(0));
				//dynamicPopulationSets.add(population);
			}
			generations++;
			...
			eachPopulations.add(solutionSet);//保留每代种群
			...
			// Obtain the current front 获取index层的个体
			...
			// Remain is less than front(index).size, insert only the best one 拥挤度较小的个体
		...
		//存储每一代的种群
		for(int i=0;i<eachPopulations.size();i++){
			eachPopulations.get(i).printObjectivesToFile(("DNSGAII_EACHPOP/"+problemSet_.get(0).getNumberOfObjectives()+"Obj_"+
					problemSet_.get(0).getName()+"_"+problemSet_.get(0).getNumberOfVariables()+"D_"+i+"gen"+".txt"));
		}
